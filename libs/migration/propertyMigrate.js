...
// --- NEW TARGET SCHEMAS (UPDATED TO MATCH UPLOADED MODELS) ---

// [UPDATED] Matches models/property.model.js
const newPropertySchema = new mongoose.Schema({
  propertyId: { type: String, unique: true },
  propertyName: String,
  contacts: {
    primary: { type: String, required: false },
    alternate: { type: String, required: false }, // [ADDED]
  },
  branch: String,
  phase: String,
  location: String,
  ...
  totalBeds: { type: Number, default: 0 },
  occupiedBeds: { type: Number, default: 0 },
  totalFloors: { type: Number, default: 0 }, // [ADDED]
  startingPrice: Number,
  sharingPrices: { type: Map, of: Number },
  deposit: { refundable: Number, nonRefundable: Number },
  propertyTitle: String,
  ...
  // [ADDED] New fields from current model
  amenities: [String],
  map: String,
  images: {
...
// [UPDATED] Matches models/room.model.js
const newRoomSchema = new mongoose.Schema({
  propertyName: { type: String, required: true },
  roomNo: { type: String, required: true },
  sharingType: { type: String, required: true },
  ...
  isHeavens: { type: Boolean, default: false },
  propertyId: { type: mongoose.Schema.Types.ObjectId, ref: "Property" },
  floorId: { type: mongoose.Schema.Types.ObjectId }, // [ADDED]
});

// --- OLD SOURCE SCHEMAS ---

// Old Property Schema (Kept as is for reading old data)
const OldPropertySchema = new mongoose.Schema(
  {
    propertyId: { type: String, unique: true, default: uuidv4 },
    propertyName: { type: String, required: true },
...
// Old Room Schema (Kept as is for reading old data)
const OldRoomSchema = new mongoose.Schema(
  {
    propertyName: { type: String, required: true },
    roomNumber: { type: String, required: true },
...
function mapOldPropertyToNewProperty(oldPropDoc) {
  ...
  const newPropData = {
    ...
    totalBeds: oldPropDoc.totalBeds,
    occupiedBeds: 0,
    totalFloors: 0, // [ADDED] Default value
    startingPrice: oldPropDoc.startingPrice,
    sharingPrices: sharingPrices,
    deposit: {
      refundable: 0,
    ...
    // [ADDED] Initializing new fields that don't exist in old DB
    amenities: [],
    map: "",
    images: {
      propertyPhotos: [],
...
function mapOldRoomToNewRoom(oldRoomDoc) {
  const newRoomData = {
    ...
    description: null,
    roomOccupants: [], // Structure updated in schema, initialized empty here
    revenueGeneration: true, // [ADDED] Default true
    isHeavens: false, // [ADDED] Default false
    floorId: null, // [ADDED] Floor relationship doesn't exist in old DB
  };
  return newRoomData;
}
