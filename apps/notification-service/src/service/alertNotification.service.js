    ...
    try {
      ...
      if (tokenRecord?.token?.length) {
        ...
        for (const token of tokenRecord.token) {
          const success = await sendPushNotificationToUser(token, notificationMessage);
          if (success) isNotificationSent = true;
        }
      }
  
      // Save alert notification
      const newAlertNotification = await AlertNotification.create(alertNotificationData);
  
      // Save notification log if sent
      if (isNotificationSent) {
        await NotificationLog.create({ userId, title, description, image: imageUrl });
      }
  
      return {
        status: 201,
        data: { message: "Alert notification added successfully!", alertNotification: newAlertNotification },
      };
    } catch (err) {
      console.error("RPC Add Alert Notification Error:", err);
      return { status: 500, message: err.message || "Failed to add alert notification" };
    }
  };
