...
export const addPettyCashController = async (req, res) => {
    try {
      ...
      const pettyCash = await sendRPCRequest( CLIENT_PATTERN.PETTYCASH.ADD_PETTYCASH,
       { 
        inHandAmount, 
        inAccountAmount, 
        manager, 
  ...
  export const getPettyCashController = async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        status: 500,
        message: "An internal server error occurred while fetching petty cash records.",
      });
    }
  };
  ...
  export const getPettyCashByManagerController = async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        status: 500,
        message: "An internal server error occurred while fetching petty cash by manager.",
      });
    }
  };