...
export const createProperty = async (req, res) => {
  ...
  const response = await sendRPCRequest(
    PROPERTY_PATTERN.PROPERTY.CREATE_PROPERTY,
    {
      state,
      city,
      location,
...
export const updateProperty = async (req, res) => {
  try {
    ...
    const response = await sendRPCRequest(
      PROPERTY_PATTERN.PROPERTY.UPDATE_PROPERTY,
      {
        id,
        ...body,
      },
...
export const deleteProperty = async (req, res) => {
  try {
    ...
    const response = await sendRPCRequest(
      PROPERTY_PATTERN.PROPERTY.DELETE_PROPERTY,
      {
        id,
      },
    );
...
export const getClientPropertiesController = async (req, res) => {
  try {
    ...
    const response = await sendRPCRequest(
      PROPERTY_PATTERN.PROPERTY.GET_CLIENT_PROPERTIES,
      {
        clientId,
      },
    );
...
export const payPropertyRent = async (req, res) => {
  try {
    const { propertyId, amount, paymentMethod, transactionId, imageUrl } =
      req.body;
    const user = req.userAuth; // Authenticated User

    console.log(
      `💰 Rent Payment Request: ${amount} for Property ${propertyId}`,
    );

    const response = await sendRPCRequest(
      PROPERTY_PATTERN.PROPERTY.PAY_RENT, // Ensure this pattern exists in your pattern file
      {
        propertyId,
        amount,
        paymentMethod,
        ...
        paidByName: `${user.firstName} ${user.lastName}`,
    ...
    console.error("Pay Rent Controller Error:", error);
    return res.status(500).json({
      success: false,
      status: 500,
      message: "Unexpected error processing rent payment",
      error: error.message,
    });
  }
};
