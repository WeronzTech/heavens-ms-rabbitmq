...
export const getAllExpensesController = async (req, res) => {
    try {
      const expenses = await sendRPCRequest(ACCOUNTS_PATTERN.EXPENSE.GET_ALL_EXPENSES, {});
  ...
  export const getExpenseByIdController = async (req, res) => {
    try {
      ...
      if (!expenseId) {
        return res.status(400).json({ success: false, status: 400, message: "Expense ID is required" });
      }
  
      const expense = await sendRPCRequest(ACCOUNTS_PATTERN.EXPENSE.GET_EXPENSE_BY_ID, { expenseId });
  ...
  export const deleteExpenseController = async (req, res) => {
    try {
      ...
      const expense = await sendRPCRequest(ACCOUNTS_PATTERN.EXPENSE.DELETE_EXPENSE, { expenseId });
      res.status(expense.status || 500).json(expense);
    } catch (error) {
  ...
  export const addExpenseCategoryController = async (req, res) => {
    try {
      ...
      const expenseCategory = await sendRPCRequest( ACCOUNTS_PATTERN.EXPENSE.ADD_EXPENSE_CATEGORY,
       { 
        mainCategory,
        subCategory}
      );
  ...
  export const getCategoryByMainCategoryController = async (req, res) => {
    try {
      ...
      if (!mainCategory) {
        return res.status(400).json({ success: false, status: 400, message: "Main category is required" });
      }
  ...
  export const getAllCategoriesController = async (req, res) => {
    try {
      const response = await sendRPCRequest(ACCOUNTS_PATTERN.EXPENSE.GET_ALL_CATEGORIES, {});
      return res.status(response.status || 500).json(response);
    } catch (error) {
  ...
  export const deleteCategoryController = async (req, res) => {
    try {
      ...
      if (!categoryId) {
        return res.status(400).json({ success: false, status: 400, message: "Category ID is required" });
      }
